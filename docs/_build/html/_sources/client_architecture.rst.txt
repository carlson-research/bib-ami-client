...
``bib-ami`` operates as a lightweight client that delegates heavy citation parsing, registry matching, and verification heuristics to the remote cloud engine. By keeping the local footprint minimal, users are guaranteed access to the latest upstream catalog integrations (CrossRef, DataCite, Open Library) without needing to update local parsing logic.
